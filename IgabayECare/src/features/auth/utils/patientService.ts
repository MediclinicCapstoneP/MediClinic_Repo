...
export const patientService = {
  ...
  async updatePatient(userId: string, updates: Partial<PatientProfile>): Promise<{ success: boolean; error?: string; patient?: PatientProfile }> {
    try {
      console.log('Updating patient for user ID:', userId, 'with updates:', updates);
      ...
      if (!patient) {
        ...
        return { success: false, error: 'Patient profile not found after update' };
      }
  ...
  async upsertPatient(data: CreatePatientData): Promise<{ success: boolean; error?: string; patient?: PatientProfile }> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to upsert patient profile';
      return { success: false, error: errorMessage };
    }
  },
}; 