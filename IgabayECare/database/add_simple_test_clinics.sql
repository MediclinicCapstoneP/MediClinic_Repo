-- Simple test clinics for Patient Home page
-- This script adds basic test data without creating users

-- Add test clinics directly
INSERT INTO public.clinics (
  id,
  user_id,
  clinic_name,
  email,
  phone,
  website,
  address,
  city,
  state,
  zip_code,
  license_number,
  year_established,
  number_of_doctors,
  number_of_staff,
  description,
  status,
  created_at,
  updated_at
) VALUES 
  (
    'test-clinic-1',
    'test-user-1',
    'OASIS DIAGNOSTIC & LABORATORY CENTER',
    'oasis@test.com',
    '+63 (555) 123-4567',
    'www.oasisdiagnostic.com',
    '123 Main Street',
    'Bogo City',
    'Cebu',
    '6010',
    'LIC-001',
    2015,
    8,
    15,
    'Comprehensive diagnostic and laboratory services with modern equipment.',
    'approved',
    now(),
    now()
  ),
  (
    'test-clinic-2',
    'test-user-2',
    'Bogo Clinical Laboratory',
    'bogo@test.com',
    '+63 (555) 234-5678',
    'www.bogoclinical.com',
    '456 Oak Avenue',
    'Bogo City',
    'Cebu',
    '6010',
    'LIC-002',
    2018,
    5,
    12,
    'Professional clinical laboratory services for accurate medical testing.',
    'approved',
    now(),
    now()
  ),
  (
    'test-clinic-3',
    'test-user-3',
    'Verdida Optical Clinic',
    'verdid@test.com',
    '+63 (555) 345-6789',
    'www.verdidoptical.com',
    '789 Health Boulevard',
    'Bogo City',
    'Cebu',
    '6010',
    'LIC-003',
    2020,
    3,
    8,
    'Professional optical services for vision care and eyewear.',
    'approved',
    now(),
    now()
  ),
  (
    'test-clinic-4',
    'test-user-4',
    'Mayol Dental Clinic',
    'mayol@test.com',
    '+63 (555) 456-7890',
    'www.mayoldental.com',
    '321 Medical Center Drive',
    'Bogo City',
    'Cebu',
    '6010',
    'LIC-004',
    2012,
    6,
    10,
    'Comprehensive dental care services for all ages.',
    'approved',
    now(),
    now()
  )
ON CONFLICT (id) DO NOTHING;

-- Add some basic specialties
INSERT INTO public.clinic_specialties (
  clinic_id,
  specialty_name,
  is_custom,
  created_at
) VALUES 
  ('test-clinic-1', 'Laboratory Services', false, now()),
  ('test-clinic-1', 'Diagnostic Tests', false, now()),
  ('test-clinic-1', 'Blood Tests', false, now()),
  ('test-clinic-2', 'Clinical Laboratory', false, now()),
  ('test-clinic-2', 'Medical Tests', false, now()),
  ('test-clinic-3', 'Optical Services', false, now()),
  ('test-clinic-3', 'Eye Care', false, now()),
  ('test-clinic-4', 'Dental Care', false, now()),
  ('test-clinic-4', 'Oral Surgery', false, now())
ON CONFLICT (clinic_id, specialty_name) DO NOTHING;

-- Check if data was inserted
SELECT 
  c.clinic_name,
  c.status,
  c.city,
  c.number_of_doctors,
  COUNT(cs.specialty_name) as specialty_count
FROM public.clinics c
LEFT JOIN public.clinic_specialties cs ON c.id = cs.clinic_id
WHERE c.status = 'approved'
GROUP BY c.id, c.clinic_name, c.status, c.city, c.number_of_doctors
ORDER BY c.created_at DESC; 