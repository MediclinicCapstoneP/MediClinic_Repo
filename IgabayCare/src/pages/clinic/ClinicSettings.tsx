...
import { Save, Camera, Clock, DollarSign, Shield, Bell, Edit, Loader2, AlertTriangle } from 'lucide-react';
...
export const ClinicSettings: React.FC = () => {
    ...
    const fetchClinicData = async () => {
      try {
        ...
        if (!currentUser || currentUser.role !== 'clinic') {
          setError('No authenticated clinic user found. Please sign in as a clinic.');
          return;
        }
        ...
        if (clinicResult.success && clinicResult.clinic) {
          ...
          if (clinicResult.clinic === undefined) {
            ...
            setError('No clinic profile found. Please complete your clinic registration or contact support.');
          } else {
            setError(clinicResult.error || 'Clinic profile not found. Please complete your clinic registration.');
          }
        }
      } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updatedData = await clinicService.updateClinic(clinicData.id, sanitizedData);
      ...
      <div className="p-6">
        <div className="flex items-center justify-center min-h-96">
          <div className="text-center">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-2">Error Loading Clinic Settings</h3>
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <Card>
            <CardContent className="p-6 text-center">
              ...
              <h3 className="text-xl font-semibold text-gray-900">{clinicData.clinic_name}</h3>
            ...
            <CardContent className="pt-0">
              <div className="space-y-3">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{stats.thisMonth} appointments</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Status</span>
                  <span className={`px-2 py-1 text-sm font-medium rounded-full ${
                    stats.status === 'approved' ? 'bg-green-100 text-green-800' :
                    stats.status === 'rejected' ? 'bg-red-100 text-red-800' :
              ...
              <div className="space-y-3">
                {days.map((day) => (
                  <div key={day.key} className="flex items-center space-x-4 p-3 bg-gray-50 rounded-lg">
                    ...
                    <Input
                      type="time"
                      value={clinicData.operating_hours[day.key as keyof typeof clinicData.operating_hours].open}
                    ...
                    <Input
                      type="time"
                      value={clinicData.operating_hours[day.key as keyof typeof clinicData.operating_hours].close}
            ...
            <CardContent className="pt-0">
              <div className="space-y-4">
                <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                  ...
                  <button 
                    ...
                    className={`w-12 h-6 rounded-full transition-colors ${
                      settings.paymentBeforeBooking ? 'bg-primary-600' : 'bg-gray-300'
                    }`}
                  >
                    <div className={`bg-white w-5 h-5 rounded-full transition-transform ${
                      settings.paymentBeforeBooking ? 'translate-x-6' : 'translate-x-0.5'
                    }`}></div>
                  </button>
                </div>
                ...
                <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                  ...
                  >
                    <div className={`bg-white w-5 h-5 rounded-full transition-transform ${
                      settings.autoValidation ? 'translate-x-6' : 'translate-x-0.5'
                    }`}></div>
                  </button>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                  ...
                  <button 
                    ...
                    disabled={!isEditing}
                    className={`w-12 h-6 rounded-full transition-colors ${
                      settings.bookingConfirmation ? 'bg-primary-600' : 'bg-gray-300'
                    }`}
                  >
                    <div className={`bg-white w-5 h-5 rounded-full transition-transform ${
                      settings.bookingConfirmation ? 'translate-x-6' : 'translate-x-0.5'
                    }`}></div>
                  </button>
                </div>
              </div>
